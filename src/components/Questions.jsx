...
function Questions() {
  ...
  const MAX_QUESTIONS_PER_CATEGORY = 20;  // correct answers to level up
  const navigate = useNavigate();

  // User preferences
  const userName = localStorage.getItem('userName');
  const lang     = localStorage.getItem('userLang');
  const [difficulty, setDifficulty] = useState(localStorage.getItem('userDifficulty'));
  ...
  const storeProgressLocally = (array) => {
    try {
      ...
      const upd = { ...prev, [lang]: { ...(prev[lang] || {}), [difficulty]: array } };
  ...
  const [correctCount, setCorrectCount] = useState(loadStoredProgress().length);
  ...
  const fetchProgressFromDB = async () => {
    try {
      ...
      if (serverProg.length !== correctIndexes.length) {
        ...
        setCorrectCount(serverProg.length);
  ...
  const saveProgressToDB = async (updatedArr) => {
    try {
      ...
      await setDoc(ref, { ...base, progress: { ...(base.progress || {}), [lang]: { ...(base.progress?.[lang] || {}), [difficulty]: updatedArr } } }, { merge: true });
    } catch (err) {
  ...
  const resetCategoryInDB = async () => {
    try {
      ...
      const snap = await getDoc(ref);
      const base = snap.exists() ? snap.data() : {};
      await setDoc(ref, { ...base, progress: { ...(base.progress || {}), [lang]: { ...(base.progress?.[lang] || {}), [difficulty]: [] } } }, { merge: true });
    } catch (err) {
  ...
  useEffect(() => {
    if (correctIndexes.length >= MAX_QUESTIONS_PER_CATEGORY) {
      ...
      if (currentIdx < levels.length - 1) {
        ...
        resetCategoryInDB(); storeProgressLocally([]);
        setCorrectIndexes([]); setCorrectCount(0); setSeenQuestions([]); setCurrentQuestionNumber(1);
        setShowEndModal(false); setShowRestartModal(false); setQuestionIndex(null);
      } else {
        navigate('/progress');
      }
    }
  }, [correctIndexes, difficulty, navigate]);
      ...
      setTime((t) => {
        if (t <= 1) {
          if (!locked) {
            ...
            setTimeout(() => { setToast(null); nextQuestionAfterTimeout(); setLocked(false); setShowAutoHint(false); }, 1000);
          }
          return 30;
        }
  ...
  const loadNextQuestion = () => {
    ...
    if (nxt === null) { setSeenQuestions([]); setShowEndModal(true); } 
    else { setSeenQuestions(prev => [...prev, nxt]); setQuestionIndex(nxt); setSelected(null); setShowHint(false); setShowAutoHint(false); setTime(30); }
  };

  const nextQuestionAfterTimeout = () => { const last = currentQuestionNumber >= MAX_QUESTIONS; setCurrentQuestionNumber(n => n + 1); if (last) setShowEndModal(true); else loadNextQuestion(); };

  const handleAnswerClick = (idx) => {
    if (selected !== null || locked) return;
    setSelected(idx); setLocked(true);
    const correctAudio = new Audio(correctSound), wrongAudio = new Audio(wrongSound);
    if (idx === question.correct) {
      correctAudio.play();
      if (!correctIndexes.includes(questionIndex)) {
        const updated = [...correctIndexes, questionIndex];
        setCorrectIndexes(updated); setCorrectCount(updated.length); storeProgressLocally(updated); saveProgressToDB(updated);
      }
      setToast({ message: '✅ תשובה נכונה!', type: 'success' });
    } else {
      wrongAudio.play(); setToast({ message: '❌ תשובה שגויה!', type: 'error' });
    }
    setTimeout(() => { setToast(null); const isLast = currentQuestionNumber >= MAX_QUESTIONS; setCurrentQuestionNumber(n => n + 1); if (isLast) setShowEndModal(true); else loadNextQuestion(); setLocked(false); }, 1500);
  };

  function splitQuestionText(text) { const heMatch = text.match(/['“״][א-ת\s_\-.,:()]+['“״]/); const hePart = heMatch ? heMatch[0] : ''; const enPart = hePart ? text.replace(hePart, '').replace(/[?؟!]/g, '').trim() : text; const punctuationMatch = text.trim().match(/[?؟!]+$/); const punctuation = punctuationMatch ? punctuationMatch[0] : ''; return { enPart, hePart, punctuation }; }
  ...
  return (
    <div dir="rtl" className="bg-blue-100 text-black dark:bg-gray-900 dark:text-white min-h-screen transition-colors duration-300">
                ...
                <main className="bg-white/90 dark:bg-gray-800 p-6 rounded-xl shadow-lg text-lg flex-grow transition-all duration-300">
                  ...
                  <ul className="space-y-2 text-right list-none p-0 m-0">
                    {question.answers.map((ans,idx)=>(
                      <button key={idx} onClick={()=>handleAnswerClick(idx)} disabled={selected!==null||locked} className={`w-full text-right p-3 rounded-lg border shadow hover:bg-blue-100 ${selected!==null?(idx===question.correct?'bg-green-400':idx===selected?'bg-red-400':''):''} ${(selected!==null||locked)?'cursor-not-allowed':''}`}>
                        {ans}
                      </button>
                    ))}
                  </ul>
                  <div className="mt-6 flex items-center justify-between">
                    <button onClick={()=>setShowHint(true)} disabled={locked} className="px-4 py-2 bg-yellow-400 text-black rounded hover:bg-yellow-500">{currentHintText}</button>
                  </div>
      ...
      {/* Toast */}
      {toast&&<div className={`fixed bottom-6 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full text-lg shadow-lg ${toast.type==='success'?'bg-green-600':'bg-red-600'} text-white`}>{toast.message}</div>}
      {/* End Modal */}
      {showEndModal&&(
